...
// with a headline, a markdown body, an optional background aside and a source link. New runs return
...
export type Story = { headline: string; body: string; context?: string; source?: string };
...
// folded background aside (`contextLabel`, "Context" or "Ne olmuştu?").
export type PaperLabels = { label?: string; contextLabel?: string };
...
export function renderPaper(paper: Paper, images: Record<string, string> = {}, labels: PaperLabels = {}) {
  ...
  for (const section of paper.sections) {
    ...
    for (const story of section.stories) {
      ...
      out.push(`<h3>${escapeHtml(story.headline)}</h3>`);
      out.push(render(story.body));
      if (story.context) {
        out.push(`<details class="aside"><summary>${escapeHtml(labels.contextLabel ?? "Context")}</summary>${render(story.context)}</details>`);
      }
