...
- `src/server.ts` is a Hono app on `@hono/node-server`, read-only apart from the run trigger: `GET /api/tasks` (names), `GET /api/tasks/:name/runs[/:runId]` (a run is `meta`, `output` and `images`; the log stays on disk; fetching one counts as a read, see the scheduler), `GET /api/status` (scheduler `off`/`on`/`paused`, last read, pause threshold), `POST /api/tasks/:name/run` (202, or 409 if that task is already running; a manual trigger for curl, the UI has no run button). Everything else is `serveStatic` over `dist/` with `index.html` as the fallback (directories, missing paths and SPA routes). Every response carries a CSP (`default-src 'self'`, images from any https host) because the reading view injects the model's markdown as HTML unsanitized. Basic auth is Hono's middleware; its 401 is an `HTTPException`, which `onError` passes through instead of turning into a 500. Binds to 127.0.0.1 only.
